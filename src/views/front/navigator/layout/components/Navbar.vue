    ...
    <div class="right-menu">
      ...
      <el-dropdown class="avatar-container right-menu-item hover-effect" trigger="click">
        ...
        <el-dropdown-menu slot="dropdown">
          <template id="" v-if = 'userInfo !== null' >
          ...
          <el-dropdown-item  @click.native="logio">
            <span>{{ userInfo? "退出" :"点击"}}登录</span>
          </el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    </div>
...
export default {
  ...
  computed: {
    ...
    getAvatar(){
      // 先加载默认头像。如果有自定义头像就替换
      this.avatar = require('@/static/images/account.png')
  ...
  methods: {
    ...
    flip() {
      if (this.whichtype === 'front'){
        if (getToken()) {
          ...
          this.$refs.login.toLogin('/layout/home')
        }
      }else{
    ...
    async logio(){
      if(this.userInfo){
        ...
        this.$refs.login.toLogin("/front/navigator/home")
      }
    },
    async logout () {
      this.$confirm('确定注销并退出系统吗？', '提示', {
        ...
        this.$store.dispatch('LogOut').then(() => {
          ...
          that.avatar = require('@/static/images/account.png')
          //  location.reload()
        })
      })
    }
